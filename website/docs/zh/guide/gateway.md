...
当多个服务商配置了**相同的模型 ID**（例如两家中转都提供 `gpt-5.5`），或在不同服务商上设置了**相同的别名**时，网关可以把该名称对外呈现为一个模型。
...
- `/v1/models` 对共享名称只展示一条裸 id（`owned_by: aqbot`），同时仍保留 `服务商/名称` 以便钉死上游。
- 使用裸 id 或共享别名请求时，按**服务商排序**选择上游，并跳过近期失败的源（限流、5xx、超时）。
- 可重试错误的故障转移发生在**流式响应开始写出之前**。

关闭时（默认）保持现有冲突展示规则，且不做多源自动切换。

随时可使用 `服务商公共ID/模型或别名` 强制指定单一上游。

### 模型别名

在 **设置 → 服务商 → 模型设置** 中，可为模型配置一个或多个**别名**。网关客户端可用别名作为 `model` 发起请求，AQBot 会将上游请求改写为真实 `model_id`。多个服务商配置相同别名时，可配合自动模型路由做多源切换。

别名主要用于**网关**；应用内对话仍使用真实模型 ID。
