...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        if manager.has_table("roles").await? {
            add_column_if_missing(manager, "enabled_mcp_server_ids_json").await?;
            add_column_if_missing(manager, "enabled_skill_names_json").await?;
        }
        Ok(())
    }
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        if manager.has_table("roles").await? {
            drop_column_if_exists(manager, "enabled_mcp_server_ids_json").await?;
            drop_column_if_exists(manager, "enabled_skill_names_json").await?;
        }
        Ok(())
    }
}
