...
import { safeParseChatMarkdown, stripAqbotTags, type ChatMarkdownNode } from '@/lib/chatMarkdown';
...
import { getLiveStreamContent, subscribeLiveStreamContent, useAgentStore } from '@/stores';
import { SyncOutlined } from '@ant-design/icons';
import { Alert, App, Tooltip, Typography, theme } from 'antd';
import {
  Check,
  ChevronDown,
  ChevronRight,
...
import { setCustomComponents, withMarkstreamComponentDisplay, type NodeComponentProps } from 'markstream-react';
...
import { ChatMarkdownRenderer, ThinkNode, getCustomAttr, type CustomNodeAttrs } from './chatMarkdownShared';
import { splitLeadingAqbotDisplayContent, stripLeadingAqbotDisplayTags } from './chatStreaming';
...
export const MINIMAP_JUMP_BEFORE_LIMIT = 4;
export const MINIMAP_JUMP_AFTER_LIMIT = 8;
export const USER_SCROLL_INTENT_GRACE_MS = 250;
...
export function shouldDeferAssistantMarkdownParse(content: string) {
...
export function stripAssistantAqbotTags(content: string) {
  return stripAqbotTags(content, { stripThink: true });
}

export function stripUserAqbotTags(content: string) {
  return stripAqbotTags(content, { stripThink: false });
}

export function PlainTextChatContent({
  content,
  textAlign,
}: {
  content: string;
...
export function MessageRenderFallback({
  content,
  notice,
  textAlign,
}: {
